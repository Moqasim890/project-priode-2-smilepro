                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        @forelse($afspraken as $afspraak)
                            @php
                                $datum = $afspraak->datum ?? null;
                                $tijd = $afspraak->tijd ?? null;
                            @endphp
                            <tr>
                                {{-- Datum met icon --}}
                                <td>
                                    <i class="bi bi-calendar3 text-muted me-1"></i>
                                    {{ $datum ? \Carbon\Carbon::parse($datum)->format('d-m-Y') : '—' }}
                                    @if($datum && \Carbon\Carbon::parse($datum)->isToday())
                                        <span class="badge bg-primary ms-1">Vandaag</span>
                                    @elseif($datum && \Carbon\Carbon::parse($datum)->isTomorrow())
                                        <span class="badge bg-info ms-1">Morgen</span>
                                    @elseif($datum && \Carbon\Carbon::parse($datum)->isPast())
                                ...
                                <td>
                                    ...
                                    {{ $tijd ? \Carbon\Carbon::parse($tijd)->format('H:i') : '—' }}
                                </td>
